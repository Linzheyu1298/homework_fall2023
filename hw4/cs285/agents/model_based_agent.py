...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        obs_delta = torch.add(next_obs, -obs)
        norl_obs_delta = (obs_delta - self.obs_delta_mean) / (self.obs_delta_std + 1e-6)
        obs_acs = torch.cat([obs, acs], dim=1)
        norl_obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-6)
        
        pre_obs_delta = self.dynamics_models[i](norl_obs_acs)
        ...
        loss = self.loss_fn(pre_obs_delta, norl_obs_delta)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        obs_acs_mean = torch.cat([obs, acs], dim=1).mean(0)
        obs_acs_std = torch.cat([obs, acs], dim=1).std(0)
        delta_mean = torch.add(next_obs, - obs).mean(0)
        delta_std = torch.add(next_obs, - obs).std(0)
        self.obs_acs_mean = obs_acs_mean
        self.obs_acs_std = obs_acs_std
        self.obs_delta_mean = delta_mean
        self.obs_delta_std = delta_std
    ...
    def get_dynamics_predictions(
        ...
        # normalizing inputs!
        obs_acs = torch.cat([obs, acs], dim=1)
        normaled_ob_ac = (obs_acs - self.obs_acs_mean )/ (self.obs_acs_std + 1e-6)
        #(bat, ob+ac)
        output = self.dynamics_models[i](normaled_ob_ac)
        #(bat, ob)
        unnuormal_out = torch.add(torch.mul(output, self.obs_delta_std + 1e-6), self.obs_delta_mean)
        pred_next_obs = torch.add(obs, unnuormal_out)
