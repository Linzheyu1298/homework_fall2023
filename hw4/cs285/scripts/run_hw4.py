...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_ob = np.mean(np.array([mb_agent.get_dynamics_predictions(i, ob, ac) for i in range(mb_agent.ensemble_size)])).squeeze(0)
        rew, done = env.get_reward(next_obs, ob)
        obs.append(ob)
        acs.append(ac)
        rewards.append(rew)
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                ...
                #print(batch)
                observations, actions, rewards, next_observations, dones = batch["observations"], batch["actions"], batch["rewards"], batch["next_observations"], batch["dones"]
                loss = mb_agent.update(i,observations, actions, next_observations)
