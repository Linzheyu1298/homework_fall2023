...
class SoftActorCritic(nn.Module):
    ...
    def get_action(self, observation: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            assert not torch.isnan(observation).any(), "Observation contains NaN"
            assert not torch.isinf(observation).any(), "Observation contains Inf"
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        #print("TYPE=", self.target_critic_backup_type)
        if self.target_critic_backup_type == "doubleq":
            
            indices = torch.arange(next_qs.size(0))
            indices[0], indices[1] = indices[1], indices[0]
            next_qs = next_qs.index_select(0,indices)
            #raise NotImplementedError
        elif self.target_critic_backup_type == "min":
            
            next_qs, next_qs = torch.min(next_qs[0], next_qs[1])
            #raise NotImplementedError
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            not_done = (torch.logical_not(done)).float()
            target_values: torch.Tensor = torch.add(reward, torch.mul(self.discount, torch.mul(next_qs, torch.add(1, not_done))))
        ...
        # Compute loss
        loss: torch.Tensor = self.critic_loss(q_values, next_qs)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        return - action_distribution.log_prob(action_distribution.rsample((self.num_actor_samples,)))
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            #print("action_dim=======:", action.shape)
            #print("want:{},{},{}".format(self.num_actor_samples,batch_size,self.action_dim,))
            assert action.shape == (
        ...
        # TODO(student)
        assert not torch.isnan(action).any(), "actions contain NaN"
        #print(action)
        eps = 1e-6
        action = torch.clamp(action, -1 + eps, 1 - eps)
        log_probs = action_distribution.log_prob(action) # the origin of NaN: distribution's paras have NaN
        assert not torch.isnan(log_probs).any(), "Logprob contains NaN"
        # num_action_samples * batch_size
        loss = - torch.mean(torch.mul(log_probs, advantage))
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        """
        assert not torch.isnan(obs).any(), "Observation contains NaN"
        assert not torch.isinf(obs).any(), "Observation contains Inf"

        if self.actor_gradient_type == "reparametrize":
            loss, entropy = self.actor_loss_reparametrize(obs)
            assert torch.isfinite(loss).all(), "Loss contains NaN, from repar"
        elif self.actor_gradient_type == "reinforce":
            loss, entropy = self.actor_loss_reinforce(obs)
            #batch_size
            assert torch.isfinite(loss).all(), "Loss contains NaN, from reinfor"

        # Add entropy if necessary
        if self.use_entropy_bonus:
            loss -= self.temperature * entropy
            #print(entropy)
            assert torch.isfinite(loss).all(), "Loss contains NaN, frome en_bonus"

        assert torch.isfinite(entropy).all(), "Entropy contains NaN"
    ...
    def update(
        ...
        for index in range(self.num_critic_updates):
        ...
        if self.target_update_period != None and step % self.target_update_period == 0:
            self.update_target_critic()
